...
class KnapsackApp(QWidget):
   ...
   def add_item(self):
        ...
        value = self.get_non_negative_float('Enter Value', 'Value:')
        if value is None:
            return
   ...
   def get_non_negative_float(self, title, label):
    while True:
        text, ok = QInputDialog.getText(self, title, label)
        if not ok:
            return None
        try:
            ...
            if value >= 0:
                ...
                self.show_error_message("Error", f"{label} must be a non-negative number.")
        except ValueError:
            self.show_error_message("Error", f"{label} must be a valid number.")
